...
xml.rss(:version => "2.0", 
  ...
  xml.channel{
    xml.title("Wiki.Cullect.com / Revisions" )
    xml.link("#{root_url}revisions") 
    xml.language("en-us")
    ...
    @revisions.each do |revision|
		...
		xml.item do			
      		xml.title revision.page.title.gsub('_', ' ')
	  		xml.link  page_base_url(:page_title => revision.page.title)
			xml.dc(:creator){xml.text! Person.find_by_cullect_id(revision.created_by).name}
