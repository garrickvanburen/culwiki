...
xml.feed "xml:lang" => "en-us", "xmlns" => "http://www.w3.org/2005/Atom" do
	xml.title("type" => "text"){xml.text! "Wiki.Cullect.com / #{@page.title.gsub('_', ' ')} "}
  	xml.link("rel" 	 => "self",  "type" => "application/xhtml+xml", 	"href" => "#{page_base_url(:page_title => @page.title)}")
  	xml.link("rel" 	 => "alternate", "type" => "application/atom+xml", 	"href" => "#{page_base_url(:page_title => @page.title)}.rss")
	
	@page.revisions.reverse.collect{|a| a.created_by}.flatten.uniq.each do |author|
		xml.author {
